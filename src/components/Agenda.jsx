...
import { Card, CardTitle, Avatar, Btn, Modal, Field, Input, Select, FormGrid, EmptyState } from './UI'
import { fmt, fmtDate, today, addDays, getWeekStart, TIPOS_SESION } from '../utils'
...
function CitaCard({ cita, paciente, onDelete }) {
  ...
  return (
    <div onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} style={{
      background: hover ? '#e1f5ee' : 'var(--teal-50)', border: '1px solid #9fe1cb',
      borderRadius: 6, padding: '5px 8px', fontSize: 11, cursor: 'default', position: 'relative', transition: 'background 0.15s',
    }}>
      ...
      {hover && (
        <button onClick={() => onDelete(cita.id)} style={{ position: 'absolute', top: 3, right: 4, fontSize: 13, color: 'var(--red-600)', background: 'none', border: 'none', cursor: 'pointer', lineHeight: 1, fontWeight: 700 }}>×</button>
      )}
    </div>
  )
}

export default function Agenda({ citas, addCita, removeCita, pacientes, sesiones, addSesion }) {
  ...
  const upcomingCitas = [...citas].filter(c => c.fecha >= todayStr).sort((a, b) => a.fecha === b.fecha ? a.hora.localeCompare(b.hora) : a.fecha.localeCompare(b.fecha)).slice(0, 10)
  ...
  async function saveCita() {
    ...
    await addCita({ ...form, pacienteId: parseInt(form.pacienteId) })
    setModalOpen(false)
  }

  async function deleteCita(id) {
    if (window.confirm('¿Eliminar esta cita?')) await removeCita(id)
  }

  async function convertirSesion(cita) {
    const pac = getPac(cita.pacienteId)
    await addSesion({ fecha: cita.fecha, pacienteId: cita.pacienteId, tipo: cita.tipo, monto: pac?.valorHabitual || 25000, pagado: 'si', notas: cita.notas })
    await removeCita(cita.id)
    setDetailCita(null)
  }
    ...
    <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 14, fontWeight: 500, marginLeft: 8, color: 'var(--text-2)' }}>{fmtDate(weekStart)} — {fmtDate(addDays(weekStart, 6))}</span>
        </div>
        <Btn variant="primary" onClick={() => { setForm({ fecha: today(), hora: '09:00', pacienteId: '', tipo: 'Rehabilitación', notas: '', recordatorio: true }); setModalOpen(true) }}>+ Nueva cita</Btn>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 16, alignItems: 'start' }}>
        <Card style={{ padding: 0, overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', borderBottom: '1px solid var(--border)' }}>
            {weekDays.map((d, i) => {
              ...
              return (
                <div key={d} style={{ padding: '10px 8px', textAlign: 'center', borderRight: i < 6 ? '1px solid var(--border)' : 'none', background: isToday ? 'var(--teal-50)' : 'var(--surface)' }}>
                  ...
                  <div style={{ fontSize: 16, fontWeight: 600, marginTop: 2, width: 28, height: 28, borderRadius: '50%', background: isToday ? 'var(--accent)' : 'transparent', color: isToday ? '#fff' : 'var(--text-1)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
                    {d.split('-')[2]}
                  </div>
                </div>
              )
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', minHeight: 380 }}>
            {weekDays.map((d, i) => {
              ...
              const isToday = d === todayStr
              return (
                <div key={d} style={{ borderRight: i < 6 ? '1px solid var(--border)' : 'none', borderTop: '1px solid var(--border)', padding: 6, background: isToday ? '#fafffe' : 'var(--surface)', display: 'flex', flexDirection: 'column', gap: 4 }}>
                  {dayCitas.length === 0 ? (
                    <div onClick={() => { setForm(f => ({ ...f, fecha: d })); setModalOpen(true) }} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', opacity: 0.3, fontSize: 18, color: 'var(--text-3)' }}>+</div>
                  ) : (
                    dayCitas.map(c => (
                  ...
                  {dayCitas.length > 0 && (
                    <button onClick={() => { setForm(f => ({ ...f, fecha: d })); setModalOpen(true) }} style={{ fontSize: 11, color: 'var(--text-3)', background: 'none', border: 'none', cursor: 'pointer', padding: '2px 0' }}>+ agregar</button>
                  )}
                </div>
              )
            })}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <Card>
            ...
            {upcomingCitas.length === 0 ? <EmptyState icon="📅" text="Sin citas próximas" /> : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {upcomingCitas.map(c => {
                  const p = getPac(c.pacienteId)
                  const isToday = c.fecha === todayStr
                  return (
                    <div key={c.id} onClick={() => setDetailCita(c)} style={{ display: 'flex', gap: 10, alignItems: 'center', padding: '9px 10px', background: isToday ? 'var(--teal-50)' : 'var(--surface-2)', borderRadius: 'var(--radius-sm)', borderLeft: `3px solid ${isToday ? 'var(--accent)' : 'var(--slate-200)'}`, cursor: 'pointer' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 11, color: 'var(--text-3)' }}>{isToday ? '🟢 Hoy' : fmtDate(c.fecha)} · {c.hora}</div>
                      </div>
